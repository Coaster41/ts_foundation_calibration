...
if __name__ == "__main__":
    ...
    for i, (input, label, forecast) in enumerate(zip(input_it, label_it, forecast_it)):
        if forecast.item_id != prev_id:
            prev_id = forecast.item_id
            print(f"id: {prev_id}, Time: {time.time()-start_time}")
        start_date = forecast.index[0] - pd.Timedelta(1, unit)
        mean_results.append([start_date, *np.mean(forecast.samples, axis=0)])
        median_results.append([start_date, *np.median(forecast.samples, axis=0)])
        for i, quantile in enumerate(quantiles):
            quantile_results[i].append([start_date, *np.quantile(forecast.samples, q=quantile/100, axis=0)])
    print("done")

    columns = ['ds', *range(1,PDT+1)]
