...
if __name__ == "__main__":
    ...
    datasets = ["police", "patents"]
    # models = ["timesfm", "moirai", "chronos", "lag-llama", "nbeats", "autoarima"]
    models = ["timesfm", "moirai", "chronos", "chronos_top_k", "chronos-bolt_top_k", "lag-llama-context", "nbeats-config", "autoarima"]
    # models = ['nbeats-context-negbinom2']
    # models = ['nbeats-config']
    # models = ['nbeats-context-normal']
    results = []
    # confidence = 0.8
    ...
    max_ids = 0
    append = True
    metrics = ['mase', 'wql', 'pse', 'mpiqr', 'msis', 'msiw', 'tce', 'cce', 'pce','stce']
    # metrics = ['msis']
    ...
    for dataset in datasets:
        for model in models:
            ...
            for (unique_id, data_df), (_, results_df), (_, quantiles_dict) in \
                ...
                if 'mpiqr' in metrics:
                    mpiqr_avg, mpiqr_h = mpiqr_mean(quantiles_dict, data_df, confidences)
                ...
                for i, confidence in enumerate(confidences):
                    ...
                    if 'mpiqr' in metrics:
                        mpiqr_avg, mpiqr_h_ = mpiqr(lower_df, upper_df, data_df, confidence)
